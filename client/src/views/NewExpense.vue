    ...
    <v-form v-on:submit.prevent="addExpense">
      <v-layout align-start row wrap>
        ...
        <v-flex color="grey darken-1" ma-5 pt-1>
          ...
          <input type="number" v-model="expense.net" required />
          <v-switch
            v-model="includeTax"
            label="Include VAT? (20%)"
            color="teal lighten-1"
          ></v-switch>
          <h4 class="input-label headline mt-5">
        ...
        <v-flex
          text-right
          mt-3
          v-show="expense.comment && expense.expenseType && expense.net > 0"
        >
...
export default {
  data() {
    return {
      ...
      items: ["Travel", "Accomodation", "Subsistence", "Other"]
    };
  },
  methods: {
    addExpense() {
    ...
    vatComputed: function() {
      if (this.includeTax === true) {
        return (this.expense.total =
          parseFloat(this.expense.net * this.expense.vat) +
          parseFloat(this.expense.net));
      } else {
