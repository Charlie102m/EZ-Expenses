  ...
  <v-form v-on:submit.prevent="addTrip">
    ...
    <v-flex md8>
      <v-alert
        ...
        type="success"
        color="teal"
      >Use the auto-complete search fields below to find you trip addresses. If you have saved your home & work addresses in the settings page, you can use the quick fill buttons to speed this process up!</v-alert>
    </v-flex>
    <v-layout align-top justify-space-around row wrap mx-1>
      <v-flex lg5>
        ...
        <v-flex mb-2 v-show="trip.distance === null">
          <v-btn
            ...
            :dark="user.homeAddress ? true : false"
            :disabled="user.homeAddress ? false : true"
            @click="setDefualtAddress('origin', 'home')"
            class="mx-5"
          >
          ...
          <v-btn
            rounded
            color="teal lighten-1"
            :dark="user.workAddress ? true : false"
            :disabled="user.workAddress ? false : true && !trip.distance"
            @click="setDefualtAddress('origin', 'work')"
          >
        ...
        <v-flex mb-2 v-show="trip.distance === null">
          <v-btn
            ...
            :dark="user.homeAddress ? true : false"
            :disabled="user.homeAddress ? false : true"
            @click="setDefualtAddress('destination', 'home')"
            class="mx-5"
          >
            <v-icon class="mr-2" dark>home</v-icon>Home
          </v-btn>
          <v-btn
            ...
            color="teal lighten-1"
            :dark="user.workAddress ? true : false"
            :disabled="user.workAddress ? false : true"
            @click="setDefualtAddress('destination', 'work')"
          >
            <v-icon class="mr-2" dark>work</v-icon>Work
          </v-btn>
        </v-flex>
        ...
        <v-flex text-right grow mt-3>
          ...
          >
            <v-icon class="mr-2" dark>directions</v-icon>
            <span>Calculate Route</span>
          </v-btn>
        </v-flex>
        <v-flex grow v-show="trip.distance !== null">
          <v-card color="teal lighten-1" class="pa-3">
            <v-card-title class="pt-0">
              <v-layout row wrap align-end justify-space-between>
                <v-flex class="white--text">
                  <h3 class="font-weight-light headline">
                    Via:
                    <strong class="font-weight-medium">{{ summary ? summary : ''}}</strong>
                  </h3>
                </v-flex>
            ...
            <v-card-text class="text-xs-left white--text font-weight-light pt-0">
              <v-layout row wrap align-center>
                <v-flex lg6 mb-4 grow text-md-left text-xs-right>
                  <h3 class="font-weight-light display-1">
                    ...
                    >{{ trip.distance ? trip.distance.toFixed(1) : 0 }}</strong> miles
                  </h3>
                </v-flex>
              ...
              <v-layout row wrap align-top>
                <v-flex md6>
                  <p>
                    <strong class="subtitle-1">From:</strong>
                  </p>
                  <p>
                    <em>{{trip.originName}}</em>
                  </p>
                  <p>{{ trip.originAddress }}</p>
                </v-flex>
                <v-flex md6>
                  <p>
                    <strong class="subtitle-1">To:</strong>
                  </p>
                  <p>
                    <em>{{trip.destinationName}}</em>
                  </p>
      ...
      <v-flex lg5>
        ...
        <v-layout align-top justify-space-around row wrap mt-3 v-show="trip.distance !== null">
          <v-flex xs4>
            <input type="number" v-show="!defualtMilageToggle" v-model="trip.milageRate" required />
            <v-switch
              v-model="defualtMilageToggle"
              v-show="defualtMilageToggle"
              label="Use default milage rate?"
              color="teal lighten-1"
            ></v-switch>
          </v-flex>
          <v-flex xs4>
...
export default {
  ...
  methods: {
      ...
      HttpService.getDirections(waypoints)
        .then(response => {
          ...
          this.trip.originAddress =
            response.data.routes[0].legs[0].start_address;
          this.trip.destinationAddress =
            response.data.routes[0].legs[0].end_address;
          this.trip.distance =
            response.data.routes[0].legs[0].distance.value / 1609.34;
          this.trip.duration = (
            response.data.routes[0].legs[0].duration.value / 60
          ).toFixed(0);
    ...
    async addTrip() {
      ...
      await HttpService.addTrip(this.trip)
        .then(response => {
          let res = response;
          if (res.status === 200 && this.autoReturn === false) {
            return this.$router.push("/trips");
          }
          return res;
        })
        .then(() => {
